...
def donchian_channels(data_high: pd.Series, data_low: pd.Series, window: int = 20) -> tuple:
    """Donchian Channels (High/Low of N periods)."""
    upper_channel = data_high.rolling(window=window).max()
    lower_channel = data_low.rolling(window=window).min()
