...
def get_current_user(credentials: HTTPBasicCredentials = Depends(security)) -> str:  # noqa: B008
...
async def index(request: Request, user: str = Depends(get_current_user)):
    ...
    return templates.TemplateResponse(
        "index.html", {"request": request, "bot_configs": bot_configs, "favourites": favourites}
    )
...
async def get_bot_list(request: Request, user: str = Depends(get_current_user)):
    bot_configs = storage.get_bot_configs()
    return templates.TemplateResponse(
        "partials/bot-list.html", {"request": request, "bot_configs": bot_configs}
    )
...
async def get_favs_partial(request: Request, user: str = Depends(get_current_user)):
    favourites = storage.get_favourites()
    return templates.TemplateResponse(
        "partials/favourites.html", {"request": request, "favourites": favourites}
    )
...
async def get_status_badge(request: Request):
    ...
    return templates.TemplateResponse(
        "partials/status-badge.html", {"request": request, "state": state}
    )
...
async def get_control_panel(request: Request):
    state = storage.get_bot_state()
    return templates.TemplateResponse(
        "partials/control-panel.html", {"request": request, "state": state}
    )
...
async def get_trades_partial(request: Request):
    ...
    return templates.TemplateResponse(
        "partials/trades.html", {"request": request, "trades": trades}
    )


@app.get("/partials/portfolio-stats", response_class=HTMLResponse)
async def get_portfolio_stats(request: Request, user: str = Depends(get_current_user)):
    metrics = storage.get_portfolio_metrics()
    return templates.TemplateResponse(
        "partials/portfolio-stats.html", {"request": request, "metrics": metrics}
    )
...
@app.post("/update")
async def update_status(request: Request, is_running: str = Form(None), strategy: str = Form(None)):
    ...
    try:
        while True:
            ...
            await websocket.send_text(
                json.dumps({"type": "log", "data": "Bot heartbeat active..."})
            )
